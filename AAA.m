function [ObjMin, BestColony] = AAA(fobj, Inputs, Targets, MaxHesaplama, LB, UB, N, D, Delta, Ap, e)

    Colonies = rand(N,D)*(UB-LB)+LB;
    ObjectiveFun = ones(N,1);
    for i=1:N
        ObjectiveFun(i) = fobj(Colonies(i,:), Inputs, Targets);
    end
    ...
    fprintf('Time = %d  ObjMin = %g  Colony = %d\n',t, ObjMin , colonyPosition);
    
    for c=1:D
        BestColony(c) = Colonies(colonyPosition,c);
    end
